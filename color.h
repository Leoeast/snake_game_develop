...
#define barrier_color 13
#define prop_color 14
#define redfood_color 15

//init_color(COLOR_BLUE , 19 , 70 , 149);
int init_game_color(){
    start_color();
    init_pair(food_color , COLOR_RED , COLOR_RED);//food
    init_pair(redfood_color , COLOR_RED , COLOR_GREEN);//redfood
    ...
    init_pair(rednum , COLOR_YELLOW , COLOR_BLUE);//red food num
    init_pair(score_one_color , COLOR_GREEN , COLOR_WHITE);//score
    init_pair(block_color , COLOR_BLACK , COLOR_BLUE);//block
    init_pair(prop_color , COLOR_WHITE, COLOR_GREEN);
    init_pair(barrier_color , COLOR_RED , COLOR_WHITE);//barrier
    return 0;
}
